...
function mskDate(){const f=new Intl.DateTimeFormat('en-CA',{timeZone:'Europe/Moscow',year:'numeric',month:'2-digit',day:'2-digit'});return f.format(new Date())}
function norm(v){return String(v??'').toLowerCase().replace(/ё/g,'е').replace(/[^a-zа-я0-9. ]/g,' ').replace(/\s+/g,' ').trim()}
function pick(o,paths,f){for(const p of paths){const v=p.split('.').reduce((a,k)=>a&&a[k],o);if(v!==undefined&&v!==null&&v!=='')return v}return f}
function toArray(d,keys=[]){if(Array.isArray(d))return d;for(const k of keys){const v=pick(d,[k]);if(Array.isArray(v))return v}return[]}
async function apiFetch(path,params={}){if(!API_KEY){const e=new Error('API_SPORT_KEY не задан в Vercel Environment Variables');e.status=500;throw e}const url=new URL(API_BASE+path);Object.entries(params).forEach(([k,v])=>{if(v!==undefined&&v!==null&&v!=='')url.searchParams.set(k,v)});const r=await fetch(url,{headers:{Authorization:API_KEY,Accept:'application/json'}});const text=await r.text();let data;try{data=text?JSON.parse(text):{}}catch{data={raw:text}}if(!r.ok){const e=new Error(`api-sport.ru HTTP ${r.status}: ${JSON.stringify(data).slice(0,350)}`);e.status=r.status;e.data=data;throw e}return data}
async function safe(path,params){try{return{ok:true,data:await apiFetch(path,params)}}catch(e){return{ok:false,error:e.message,status:e.status,data:e.data}}}
async function getSports(){const d=await apiFetch('/sport');return toArray(d,['sports','data','items','response','result'])}
